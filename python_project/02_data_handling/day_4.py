...
3. Store the following in a CSV file (`weather_log.csv`):
    - Data (auto-filled as today's data)
    - City
    - Temperature (in °C)
...
5. Allow users to:
    - Add new weather log
    - View all logs
    - Show average, highest, lowest temperatures, and most frequent conedition
...
API_KEY =  "5e06990a51fa3a4a5ab15a7940326e37"
    ...
    with open(FILENAME, 'r', newline='', encoding="utf-8") as f:
        ...
        for row in reader:
            if row["Date"] == date and row["City"].lower() == city.lower():
                print("Enter for this city and date exists")
                return
...
def view_logs():
    with open(FILENAME, 'r', encoding="utf-8") as f:
        ...
        for row in reader[1:]:
            ...
            # print("DEBUNG row= ", row, "length =", len(row))
            if len(row) != 4:  
                 # skip bad rows
                continue
            print(f"{row[0]} : {row[1]} : {row[2]} : {row[3]}")
